...
function obrigatorio(e) {
	...
	if ( e.currentTarget.value.length <=0 ) {
		...
		if ( e.currentTarget.id == "cepJ" ) {
			var regexCep = /^\d+\-\d+$/;
			if (!e.currentTarget.value.match(regexCep) || e.currentTarget.value.length < 8) { 
...
var x = _gbc("tSpan");/*class de todas as tags span*/
	...
	/*FIM EFEITO MENU*/
        ...
        var titlePag = _$("titlePag").textContent;
        document.title = titlePag;
...
_$("formularioJ").onsubmit = function(e){
	...
	for (var i = 0; i < y.length; i++){
		...
		if ( y[i].value.length <= 0 || x[i].innerHTML.length > 0 ) {
				...
				e.preventDefault();
			if (err){
				var l = document.querySelector("[for="+ y[i].id +"]").textContent;
				alert("Verifique se digitou corretamente o campo " + l);
				y[i].focus();
				y[i].select();
				err = true;
				break
